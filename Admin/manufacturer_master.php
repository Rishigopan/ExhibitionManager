...
<body>
    ...
    <div class="wrapper">
            ...
            <div class="offcanvas-body">
                ...
                <div id="Menu_options">
                    <ul class="list-unstyled">
                        ...
                        <li class=" <?php if($_COOKIE['custtypecookie'] == 'SuperAdmin' || $_COOKIE['custtypecookie'] == 'Admin'){} else{ echo "d-none" ;} ?>">
                            <a href="./manufacturer_master.php" class="active">
                                <i class="material-icons">home</i>
        ...
        <div class="container main-content">
            ...
            <h3 class="mt-2 title shadow-sm  py-2 text-center">Add Manufacturer</h3>

            <div class="card card-body main-card shadow-sm">
                <div id="add_manufacturer">
                    <div class="row">
                        <div class="col-md-5 product_details  px-xl-2">
                            <form action="" id="addManufacturerForm" novalidate>
                                <div class="inputs">
                                    <label class="form-label" for="Manufacturer_name">Manufacturer Name</label>
                                    <input type="text" class="form-control" id="Manufacturer_name" placeholder="" name="manufacturer_name" data-v-max-length="20" data-v-message="Maximum 20 characters" required>
                                </div>
                            ...
                            <form action="" id="updateManufacturerForm" style="display: none;" novalidate>
                                <div class="inputs">
                                    <label class="form-label" for="updateManufacturer_name">Manufacturer Name</label>
                                    <input type="text" id="editManufacturerId" name="UpdateManufacturerId" hidden>
                                    <input type="text" class="form-control" id="updateManufacturer_name" placeholder="" name="updateManufacturer_name" data-v-max-length="20" data-v-message="Maximum 20 characters" required>
                                </div>
                        ...
                        <div class="col-md-7 p-0  px-xl-4">
                            <div class="card card-body view_details p-0">
                                ...
                                <ul class="list-unstyled px-1" id="displayManufacturer">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(function() {
                ...
                let validator = $('#addManufacturerForm').jbvalidator({
                ...
                validator.validator.custom = function(el, event) {
                    if ($(el).is('#Manufacturer_name') && $(el).val().trim().length == 0) {
                ...
                $(document).on('submit', '#addManufacturerForm', (function(e) {
                    e.preventDefault();
                    var ManufacturerData = new FormData(this);
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(data) {
                            ...
                            if (response.addmanufacturer == "0") {
                                toastr.warning("Manufacturer Already Exists");
                            } else if (response.addmanufacturer == "1") {
                                toastr.success("Successfully Added Manufacturer");
                                $('#addManufacturerForm')[0].reset();
                                getManufacturerData();
                                $('#Manufacturer_name').focus();
                            } else if (response.addmanufacturer == "2") {
            ...
            /* update manufacturer Start */
            $(function() {

                let validator = $('#updateManufacturerForm').jbvalidator({
                    language: 'dist/lang/en.json',
                    successClass: false,
                    html5BrowserDefault: true
                });
                ...
                validator.validator.custom = function(el, event) {
                    if ($(el).is('#updateManufacturer_name') && $(el).val().trim().length == 0) {
                        return 'Cannot be empty';
                    }
                }
                ...
                $(document).on('submit', '#updateManufacturerForm', (function(e) {
                    e.preventDefault();
                    var updateManufacturerData = new FormData(this);
                    $.ajax({
                        type: "POST",
                        url: "master_operations.php",
                        data: updateManufacturerData,
                        beforeSend: function() {
                            $('#updateManufacturerForm').addClass("disable");
                        },
                        success: function(data) {
                            console.log(data);
                            $('#updateManufacturerForm').removeClass("disable");
                            var updateresponse = JSON.parse(data);
                            if (updateresponse.updatemanufacturer == "0") {
                                toastr.warning("Manufacturer Already Exists");
                            } else if (updateresponse.updatemanufacturer == "1") {
                                toastr.success("Successfully Updated Manufacturer");
                                $('#updateManufacturerForm')[0].reset();
                                getManufacturerData();
                                $('#updateManufacturerForm').hide();
                                $('#addManufacturerForm').show();
                                $('#Manufacturer_name').focus();
                            } else if (updateresponse.updatemanufacturer == "2") {
                                toastr.error("Some Error Occured");
                            } else {
                                toastr.error("Some Error Occured");
                            }
